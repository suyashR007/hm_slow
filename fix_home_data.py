...
# All unique products extracted from the H&M Home page HTML provided by the user.
# Each product has a UNIQUE image - no repeats.
products = [
    {
        "id": "1249274005",
        "name": "Reed diffuser",
...
for p in products:
    if p['id'] not in seen_ids:
        seen_ids.add(p['id'])
        img_key = p['image'].split('?')[0]  # strip query params for comparison
        if img_key not in seen_images:
            seen_images.add(img_key)
            unique_products.append(p)
        else:
            print(f"  Skipping duplicate image for: {p['name']} ({p['id']})")
    else:
        print(f"  Skipping duplicate id: {p['id']}")

print(f"\nTotal unique products (no repeated images): {len(unique_products)}")

# Now replace the home array in products-data.js
target_file = os.path.join('js', 'products-data.js')
with open(target_file, 'r', encoding='utf-8') as f:
    content = f.read()

# Find and remove the old "home" array
# Pattern: ,"home": [...]} };  OR  , "home": [...] } };
# We need to find "home": [ ... ] and the closing braces
home_start = content.find('"home"')
if home_start == -1:
    print("ERROR: Could not find 'home' key in products-data.js")
    exit(1)

# Find the comma before "home"
comma_pos = content.rfind(',', 0, home_start)

# Find the end of the file (closing braces)
# The file ends with } };
file_end_pattern = re.search(r'\}\s*\};?\s*$', content)
if not file_end_pattern:
    print("ERROR: Could not find closing braces")
    exit(1)

# Build new content: everything before the comma + new home array + closing
before_home = content[:comma_pos]

# Format the new home array
home_json = json.dumps(unique_products, indent=8, ensure_ascii=False)

new_tail = ',\n    "home": ' + home_json + '\n  }\n};\n'

new_content = before_home + new_tail

with open(target_file, 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f"Successfully replaced home data with {len(unique_products)} unique products!")
print("Each product now has its own unique image.")
