...
if os.path.exists('new_kids_dump.html'):
    try:
        with open('new_kids_dump.html', 'r', encoding='utf-8', errors='ignore') as f:
            ...
            # Loose Parse (Split by article/div/a with title)
            # Find all title="..."
            # Assume image is in the same block (following or preceding slightly)
            # Actually, extracting ALL jpg urls and map to nearest title?
            
            # Regex for Title + Image
            # title="NAME" ... src="URL"
...
for p in kids_products:
    ...
    if img_url:
        ...
        print(f"FAILED: '{p_name}' -> '{norm_target}'")
